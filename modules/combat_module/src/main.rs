...
//! - KillAura (multi-target, post timing)
//! - Aim (head snap, GCD, patterns)
//! - AutoClicker (CPS, statistics)
//! - Reach (distance validation)
//! - NoSwing (missing arm animation)
...
use combat_module::{
    CombatChecks, CombatConfig, Finding, ParsedPacket, PlayerState,
...
impl From<Finding> for FindingDto {
    fn from(f: Finding) -> Self {
        Self {
            ...
            detector_name: format!("combat_{}", f.feature_id.name()),
...
async fn main() -> anyhow::Result<()> {
    ...
    let module_name = std::env::var("MODULE_NAME").unwrap_or_else(|_| "combat_module".to_string());
    ...
    tracing::info!("Starting Combat Module v0.1.0");
...
async fn ingest(
    ...
    // Run checks
    let checks = CombatChecks::new(state.config.clone());
