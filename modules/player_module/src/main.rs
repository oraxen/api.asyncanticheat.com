...
//! - BadPackets (invalid pitch, NaN, abilities spoof, invalid slots, flying flood)
//! - Scaffold (airborne bottom-face placement, sprinting while bridging)
...
use player_module::{
    PlayerChecks, PlayerConfig, Finding, ParsedPacket, PlayerState,
...
impl From<Finding> for FindingDto {
    fn from(f: Finding) -> Self {
        Self {
            ...
            detector_name: format!("player_{}", f.feature_id.name()),
...
async fn main() -> anyhow::Result<()> {
    ...
    let module_name = std::env::var("MODULE_NAME").unwrap_or_else(|_| "player_module".to_string());
    ...
    tracing::info!("Starting Player Module v0.1.0");
...
async fn ingest(
    ...
    // Run checks
    let checks = PlayerChecks::new(state.config.clone());
