...
pub async fn get_modules(
    ...
    // Best-effort migration: replace legacy default modules (AAC/Vulcan) with category modules.
    ...
    if let Ok(mut tx) = state.db.begin().await {
        let _ = sqlx::query(
            ...
            where server_id = $1
              and (
                (lower(name) like '%aac%' and (base_url like 'http://127.0.0.1:4011%' or base_url like 'http://localhost:4011%'))
                or (lower(name) like '%vulcan%' and (base_url like 'http://127.0.0.1:4012%' or base_url like 'http://localhost:4012%'))
              )
            "#,
        )
        .bind(&server_id)
...
pub async fn get_status(
    ...
    // Plugin uploads batches when there's data. With no players, only PING/PONG
    // keep-alive packets accumulate (~hourly). Use 2-hour threshold for "online".
    let plugin_online = plugin_last_seen_ms < 2 * 60 * 60 * 1000; // Online if seen within 2h
