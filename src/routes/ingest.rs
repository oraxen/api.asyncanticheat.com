...
/// (AAC/Vulcan) so older servers don't keep showing outdated module names in the dashboard.
async fn ensure_default_modules(db: &PgPool, server_id: &str) -> Result<(), sqlx::Error> {
    ...
    // Remove them so the dashboard doesn't show outdated "aac/vulcan" modules forever.
