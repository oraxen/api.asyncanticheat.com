...
/// results in empty dashboard data. We add the built-in AAC + Vulcan modules on first ingest.
async fn ensure_default_modules(db: &PgPool, server_id: &str) -> Result<(), sqlx::Error> {
    ...
    // AAC module (port 4011) and Vulcan module (port 4012).
    ...
    sqlx::query(
        ...
        values
            ($1, 'AAC Module', 'http://127.0.0.1:4011', true, 'raw_ndjson_gz', now(), now()),
            ($1, 'Vulcan Module', 'http://127.0.0.1:4012', true, 'raw_ndjson_gz', now(), now())
        "#,
    )
    .bind(server_id)
    .execute(&mut *tx)
